...
local function validateColor(color, name)
    ...
    if model == "grayscale" then
        ...
        assert(color.white >= 0 and color.white <= 1,
               name .. ".white must be 0..1")
    else
...
local function validateConfig(config)
    ...
    assert(validHotkeyKey(config.hotkey.key),
    ...
    assert(colorModel(config.theme.fallbackAccent) == "rgb",
    ...
    assert(validHotkeyKey(config.navigation.cancelKey),
    ...
    if config.navigation.enabled then
        ...
        assert(validHotkeyKey(config.navigation.activateKey),
               "Gearbox: invalid navigation.activateKey: " ..
                   config.navigation.activateKey)
        assert(keyIdentity(config.navigation.activateKey) ~=
                   keyIdentity(config.navigation.cancelKey),
               "Gearbox: navigation activate and cancel keys must differ")
        assert(keyIdentity(config.navigation.activateKey) ~= "up" and
                   keyIdentity(config.navigation.activateKey) ~= "down",
